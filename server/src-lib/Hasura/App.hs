...
initialiseAppEnv env BasicConnectionInfo {..} serveOptions@ServeOptions {..} liveQueryHook serverMetrics prometheusMetrics traceSamplingPolicy = do
  ...
  void $ Kronor.startInvalidTokensListenerThread logger metadataDbPool (fromIntegral soTokenPollInterval) invalidTokensRef
  (tracer, _) <- allocate
...
instance Kronor.HasInvalidTokens (ExceptT QErr AppM) where
