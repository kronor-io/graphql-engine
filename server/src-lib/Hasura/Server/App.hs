...
mkSpockAction appStateRef qErrEncoder qErrModifier apiHandler = do
  ...
  tracingCtx <- liftIO do
    -- At kronor, we don't actually use the hasura-generated span and trace ids,
    -- we have actaully rewritten the frollowing two functions to always return the same value
    -- for traceId and spanId. This is because we want to use the traceId and spanId
    -- generated by the opentelemetry library
    pure $
      Tracing.TraceContext
        appEnvTracer
        Tracing.fixedTraceId
        Tracing.fixedSpanId
        Nothing
